...
function UpdateSpareParts() {
  ...
  useEffect(() => {
    axios
      .get(BASE_URL + "/sparePart/fetchTableEdit", {
        params: {
          id: id,
        },
      })
  ...
  useEffect(() => {
    axios
      .get(BASE_URL + "/supp_SparePart/fetchTableEdit", {
        params: {
          id: id,
        },
      })
  ...
  const handleSelectChange = (selectedOptions) => {
    ...
    const updatedTable = [
      ...tableSupp.filter((row) =>
        selectedOptions.some((option) => option.value === row.supplier_code)
      ),
      ...selectedOptions
        .filter(
          (option) =>
            !tableSupp.some((row) => row.supplier_code === option.value)
        )
        .map((option) => ({
    ...
    if (form.checkValidity() === false) {
      ...
      axios
        ...
        .then((res) => {
          if (res.status === 200) {
            swal({
              title: "The Product Spare Part Update Successful!",
            ...
            swal({
              icon: "error",
              title: "Product Spare Part Already Exist",
              text: "Please input another Product Spare Part",
            });
          } else {
            swal({
              icon: "error",
