...
function BIS_Summary({ authrztn }) {
  ...
  const handleSearch = (event) => {
    ...
    const filteredSearchBIS = searchBIS.filter((data) => {
      const costCenterNameMatches = data.costCenterName
        .toLowerCase()
        .includes(searchTerm);
      const categoryMatches = Object.values(data.prices).some((price) =>
        price.toString().toLowerCase().includes(searchTerm)
      );
  ...
  const exportToCSV = () => {
    ...
    bisContent.forEach((data) => {
      const rowData = [
        ...
        ...Object.values(data.prices).map((price) =>
