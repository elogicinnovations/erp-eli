...
const ActivityLog = ({ authrztn }) => {
  ...
  const handleRowToggle = async (masterlist_id) => {
    try {
      ...
      setOpenRows((prevOpenRow) =>
        prevOpenRow === masterlist_id ? null : masterlist_id
      );
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSearch = (event) => {
    ...
    const filteredData = originalUserActivity.filter((data) => {
      const nameMatch = data.masterlist.col_Fname
        .toLowerCase()
        .includes(searchTerm);
      const dateMatch = formatDate(data.maxCreatedAt)
        .toLowerCase()
        .includes(searchTerm);
  ...
  const handleGoButtonClick = () => {
    ...
    const filteredData = originalUserActivity.filter((data) => {
      ...
      const isMatchingUser =
        ...
        data.masterlist.col_Fname === selecteduser;
    ...
    try {
      ...
      if (dataForExport.length > 0) {
        ...
        const csvContent =
          ...
          dataForExport
            .map((data) => {
              return `${data.masterlist.col_Fname},${
                data.masterlist.department.department_name
              },${data.action_taken},${formatDate(data.createdAt)}`;
            })
            .join("\n");
                      ...
                      {currentItems.map((data, i) => (
                        <React.Fragment key={i}>
                          <tr>
                            <td>
                              <IconButton
                                ...
                                size="small"
                                onClick={() =>
                                  handleRowToggle(data.masterlist_id)
                                }
                              >
                          ...
                          <tr>
                            ...
                            >
                              <Collapse
                                in={openRows === data.masterlist_id}
                                timeout="auto"
                                unmountOnExit
                              >
                                <div
                                  style={{ width: "95%" }}
                                  id="specificUserTable"
                                >
                                  <thead
                                    style={{
