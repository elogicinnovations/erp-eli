...
function ViewSpare({ authrztn }) {
  ...
  useEffect(() => {
    axios
      .get(BASE_URL + "/inventory/fetchWarehouseInvetory_spare", {
        params: {
          id: id,
        },
      })
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      // console.log('code' + id)
      axios
        .get(BASE_URL + "/inventory/fetchView_spare", {
          params: {
            id: id,
          },
        })
        .then((res) => {
          ...
          setBinLocation(
            `${res.data[0].binLocation.bin_subname}_${res.data[0].binLocation.bin_name}`
          );
          setUm(res.data[0].spareParts_unitMeasurement);
          setDetails(res.data[0].sparePart_desc);
