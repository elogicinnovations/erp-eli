  ...
  try {
    ...
    [data_prd].forEach((data) => {
      data.forEach((data) => {
        ...
        const price =
          (mappingInventory.price +
            mappingInventory.freight_cost +
            mappingInventory.custom_cost) *
          data.quantity;

        // Find the index of the Cost Center in dataArray
        const costCenterIndex = dataArray.findIndex(
          (item) => item.costCenterId === costCenterId
        );
